    ...
    $.fn.jsonTree = function (data, setting) {
        ...
        function createTreeItem(jsonData, $ul) {
            ...
            if (jsonData.Title && jsonData.ItemId) {
				...
				var $ahref = $("<a></a>").appendTo($span).attr("href","/#workflow.debugger/"+jsonData.ItemId).addClass('worflowLink');
				 if (jsonData.Parent && setting.page == "listPage"){
					var $aDel =  $("<a></a>").appendTo($li).attr("data-id",jsonData.ItemId).addClass('worflowLinkDelete trash').attr("title", "delete");
					$("<u></u>").appendTo($aDel).addClass('fa fa-trash');
                 }
                 if (jsonData.Parent && setting.page == "listPage"){
					var $aClone =  $("<a></a>").appendTo($li).attr("href", "/workflow/cloning.download/"+ jsonData.ItemId).addClass('worflowClone wClone').attr("title", "Clone "+ jsonData.Title);
					$("<u></u>").appendTo($aClone).addClass('fa  fa-clone');
				 }
                var $icon = $("<i></i>").appendTo($span);
                $ahref.append(jsonData.Title);
        ...
        if (setting.forceCreate || !$this.data('treeView')) {
            ...
            $hasSubElements.addClass('parent_li').find(' > span i').attr('title', 'Expand workflow');//.addClass("hasDropWorkflowTree");
            ...
            $('ul', $this).on('click', 'li > span', function (e) {
                ...
                if ($closestLi.hasClass("parent_li") && e.target && e.target.nodeName == "I") {
                    ...
                    if (children.is(":visible")) {
                        children.hide('fast');
						 $(this).find(' > i').addClass('icon-plus-sign').removeClass('icon-minus-sign').attr('title', 'Expand workflow');
                    } else {
                        children.show('fast');
						$clickedItem.find(' > i').addClass('icon-minus-sign').removeClass('icon-plus-sign').attr('title', 'Collapse workflow');
                    }
                } else {
